...
data <- data %>% 
  ...
  mutate(indexGFR.q = ntile(indexGFR, 5)) %>% # make quintiles
  mutate(indexGFR.q = as.factor(indexGFR.q)) %>%
...
# Calculate SMDs for tx groups  
  ...
  # Create SMD tables within each indexGFR level
    indexGFR.values <- 1:5
    source("./functions/overlap-eval.R")
    smd.tab.list <- lapply(indexGFR.values, function(indexGFR.value) {
      subset <- data.smdtab %>% filter(indexGFR.q == indexGFR.value)
  ...
  # Plot density curves in subsets
    density.plots.list <- lapply(indexGFR.values, function(indexGFR.value) {
      subset <- data.factor %>% filter(indexGFR.q == indexGFR.value)
    ...
    num.summary.list <- lapply(indexGFR.values, function(indexGFR.value) {
      subset <- data.factor %>% filter(indexGFR.q == indexGFR.value)
...
# Step 3: Estimate Weights
  ...
  table(data$indexGFR.q)
  Z <- data$indexGFR.q # a factor
...
# Step 4: assess balance, weights, and ESS
  ...
  # ATT (Balwts)
    bal.plots.ATT <- bal.plots(data, "ATTwts", "indexGFR.q", 'pain', covs) 
    bal.plots.ATT_clean <- bal.plot.clean(data = data, weights = "ATTwts", strata = "indexGFR.q", 
                                          treatment = "pain", covs = covs, subset = TRUE, main.title = "Balance Plot eGFR")
    # Save plots
    if (!dir.exists("./results/eGFR/balplots")) {
      dir.create("./results/eGFR/balplots", recursive = TRUE)
    }
     for (i in seq_along(bal.plots.ATT_clean)) {
      plot_i <- bal.plots.ATT_clean[[i]]
      file_name <- paste0("./results/eGFR/balplots/balance_plot_egfr_quintile_", i, ".jpeg")
      ggsave(filename = file_name, plot = plot_i, device = "jpeg", 
             width = 7, height = 9, units = "in", dpi = 300)
    }
    composite.plot <- composite.bal.plot(bal.plots.ATT_clean, main.title = "Balance Plots by eGFR Quintile")
    composite.plot
    ggsave(filename = "./results/eGFR/balplots/Composite-eGFR.jpeg", plot = composite.plot, device = "jpeg", 
  ...
  # Examine Balance in continuous covariate distributions
    # ATT
      ...
      age.density.plots.list <- lapply(indexGFR.values, function(indexGFR.value) {
        subset <- data %>% filter(indexGFR.q == indexGFR.value)
      ...
      indexGFR.density.plots.list <- lapply(indexGFR.values, function(indexGFR.value) {
        subset <- data %>% filter(indexGFR.q == indexGFR.value)
      ...
      bmi.density.plots.list <- lapply(indexGFR.values, function(indexGFR.value) {
        subset <- data %>% filter(indexGFR.q == indexGFR.value)
...
# Step 5: Export to Stata
  ...
  spline_vals <- predict(ns(data$indexGFR, df = 4, intercept = FALSE), newx = seq(20, 150, by = 10))
  rownames(spline_vals) <- c("eGFR20", "eGFR30", "eGFR40", "eGFR50", "eGFR60",
                             "eGFR70", "eGFR80", "eGFR90", "eGFR100", "eGFR110",
                             "eGFR120", "eGFR130", "eGFR140", "eGFR150")
  spline_vals
