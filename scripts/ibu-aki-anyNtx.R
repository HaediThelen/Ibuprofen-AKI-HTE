...
# Ibuprofen effect on AKI - CHF HTE study 
# Step 0: Prep
# Load in data
data <- read_dta("./data/ibu-aki-data.dta") 

# Clean data and and prep for balanceR, make nephrotoxic exposure 
data <- data %>% 
...
# Breakdown of treatment and anyNtx  
  data %>% group_by(pain, anyNtx) %>% summarise(n=n())
...
# Create SMD table overall
  source("./functions/overlap-eval.R")
  smd.tab <- smd.table(data.smdtab, covs)

# Create SMD tables within each anyNtx level
  anyNtx.values <- 0:1 
  source("./functions/overlap-eval.R")
  smd.tab.list <- lapply(anyNtx.values, function(anyNtx.values) {
    subset <- data %>% filter(anyNtx == anyNtx.values)
...
# Plot density curves in subsets
  density.plots.list <- lapply(anyNtx.values, function(anyNtx.values) {
    subset <- data.factor %>% filter(anyNtx.values == anyNtx.values)
...
# Compare Number Summaries of continuous covariates  in subsets 
  num.summary.list <- lapply(anyNtx.values, function(anyNtx.values) {
    subset <- data.factor %>% filter(chf.cat == anyNtx.values)
...
# Step 3: Estimate Weights
  table(data$pain)
  # Reatin abxNtTX, ntxOther, vancoBase in this list because they are confounders
  # in anyNtx=1 subgroup, though they do not vary in anyNtx = 0 subgroup
...
# Table 1 for ATT overall
  ...
  tab.1.anyNtx <- table.1(data, "ATTwts", covs) # one equal probability warning is expected

# SMD Plots and Total Bias Reduction 
  detach(package:Hmisc, unload=TRUE)
  source("./functions/balance-plots.R")
# ATT (Balwts)
  bal.plots.ATT.clean <- bal.plot.clean.bin(data = data, weights = "ATTwts", strata = "anyNtx.cat", 
                                          treatment = "pain", covs = covs, subset = TRUE, main.title = "Balance Plot Nephrotoxins")

# vancoBase, ntxOther, and abxNtx are missing for anyNtx = 0 group because they do not vary there.
# Save plots
  for (i in seq_along(bal.plots.ATT.clean)) {
    plot_i <- bal.plots.ATT.clean[[i]]
    file_name <- paste0("./results/anyNtx/balPlots/balance_plot_anyNtx_", i, ".jpeg")
    ggsave(filename = file_name, plot = plot_i, device = "jpeg", 
           width = 7, height = 9, units = "in", dpi = 300)
  }
  composite.plot <- composite.bal.plot.bin(bal.plots.ATT.clean, main.title = "Balance Plots by Nephrotoxin Status", strata = "anyNtx.cat")
  composite.plot
  
  ggsave(filename = "./results/anyNtx/balPlots/Composite-anyNtx.jpeg", plot = composite.plot, device = "jpeg", 
...
# write to data folder
  write.dta(data, "./data/ibu-aki-anyNtx.dta") 
