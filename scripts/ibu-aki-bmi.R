...
data <- read_dta("/Users/haedi/Library/CloudStorage/Box-Box/Data/NSAID-AKI/data/ibu-aki-data.dta")
  ...
  data <- data %>% 
    ...
    mutate(bmi.q = ntile(bmi, 5)) %>% # make quintiles
    mutate(bmi.q = as.factor(bmi.q)) %>%
...
# 2. Check for positivity/overlap
  ...
  # Create SMD tables within each BMI level
      ...
      smd.tab.list <- lapply(bmi.values, function(bmi.value) {
        subset <- data.smdtab %>% filter(bmi.q == bmi.value)
  ...
  # Plot density curves in subsets
    density.plots.list <- lapply(bmi.values, function(bmi.value) {
      subset <- data.factor %>% filter(bmi.q == bmi.value)
  ...
  # Compare Number Summaries of continuous covariates  in subsets 
    num.summary.list <- lapply(bmi.values, function(bmi.value) {
      subset <- data.factor %>% filter(bmi.q == bmi.value)
...
# Step 4: assess balance, weights, and ESS
  ...
  # SMD Plots and Total Bias Reduction 
    ...
    # ATT (Balwts)
      bal.plots.ATT <- bal.plots(data, "ATTwts", "bmi.q", 'pain', covs) 
      bal.plots.ATT_clean <- bal.plot.clean(data = data, weights = "ATTwts", strata = "bmi.q", 
                                            treatment = "pain", covs = covs, subset = TRUE, main.title = "Balance Plot BMI")
      # Save plots
      if (!dir.exists("./results/bmi/balplots")) {
        dir.create("./results/bmi/balplots", recursive = TRUE)
      }
      for (i in seq_along(bal.plots.ATT_clean)) {
        plot_i <- bal.plots.ATT_clean[[i]]
        file_name <- paste0("./results/bmi/balplots/balance_plot_BMI_quintile_", i, ".jpeg")
        ggsave(filename = file_name, plot = plot_i, device = "jpeg", 
               width = 7, height = 9, units = "in", dpi = 300)
      }
      composite.plot <- composite.bal.plot(bal.plots.ATT_clean, main.title = "Balance Plots by BMI Quintile")
      ggsave(filename = "./results/bmi/balplots/Composite-BMI.jpeg", plot = composite.plot, device = "jpeg", 
 ...
 # Examine Balance in continuous covariate distributions
      ...
      age.density.plots.list <- lapply(bmi.values, function(bmi.value) {
        subset <- data %>% filter(bmi.q == bmi.value)
      ...
      indexGFR.density.plots.list <- lapply(bmi.values, function(bmi.value) {
        subset <- data %>% filter(bmi.q == bmi.value)
      ...
      bmi.density.plots.list <- lapply(bmi.values, function(bmi.value) {
        subset <- data %>% filter(bmi.q == bmi.value)
...
# Step 5: Export to Stata
   ...
   #Make a spline for BMI for continuous BMI analysis
      bmi.sp <- ns(data$bmi, df=3, intercept=FALSE) # fit natural cubic spline for bmi
      nos <- seq(1:ncol(bmi.sp)) # number of cols in bmi.sp
      colnames(bmi.sp) <- paste("bmi", nos, sep="") # rename the columns
      data <- cbind(data, bmi.sp) # add the spline columns to the data
  ...
  rownames(spline_vals) <- c("BMI15", "BMI20", "BMI25", "BMI30", "BMI35",
                             "BMI40", "BMI45", "BMI50")
  spline_vals
