...
table.1 <-function(data, wts=NULL, covs) {
  ## Make Table 1
  ...
  data.tab1 <- data %>%
    ...
    mutate(center.hup = factor(center.hup, labels = c("Not HUP", "HUP"))) %>%
    mutate(center.presb = factor(center.presb, labels = c("Not Presbyterian", "Presbyterian"))) %>%
    mutate(center.pa = factor(center.pa, labels = c("Not Pennsylvania Hospital", "Pennsylvania Hospital"))) %>%
    mutate(presentation.ed= factor(presentation.ed, labels = c("Not ED", "ED"))) %>%
    mutate(presentation.icu= factor(presentation.icu, labels = c("Not ICU", "ICU"))) %>%
    mutate(presentation.or= factor(presentation.or, labels = c("Not OR", "OR"))) %>%
    mutate(presentation.floor= factor(presentation.floor, labels = c("Not Floor", "Floor"))) %>%
    ...
    mutate(abBlocker = factor(abBlocker, labels = c("No", "Yes"))) %>%
    mutate(hctzBase = factor(hctzBase, labels = c("No", "Yes"))) %>%
    mutate(hydralazineBase  = factor(hydralazineBase , labels = c("No", "Yes"))) %>%
    mutate(loopBase = factor(loopBase, labels = c("No", "Yes"))) %>%
    mutate(htnOther = factor(htnOther, labels = c("No", "Yes"))) %>%
    mutate(sup.no = factor(sup.no, labels = c("No", "Yes"))) %>%
    mutate(sup.h2ra = factor(sup.h2ra, labels = c("No", "Yes"))) %>%
  ...
  label(data.tab1$indexGFR) <- "GFR ml/min per 1.73m^2 "
  label(data.tab1$preAkiStatus) <- "Prior AKI"
  label(data.tab1$wbcBase) <- "WBC, x10^8 cells/L"
  label(data.tab1$hgbBase) <- "Hemoglobin, g/dL"
  label(data.tab1$platBase) <- "Platelets, x10^11 cells/L"
  label(data.tab1$labclBase) <- "Chloride, mEq/L"
  label(data.tab1$labkBase) <- "Potassium, mEq/L"
  ...
  label(data.tab1$abBlocker) <- "Combined Alpha and Beta Blocker"
  label(data.tab1$hctzBase) <- "Hydrochlorothiazide"
  label(data.tab1$hydralazineBase ) <- "Hydralazine"
  label(data.tab1$loopBase) <- "Loop Diuretics"
  label(data.tab1$htnOther) <- "Other Antihypertensives"
  label(data.tab1$sup.no) <- "Acid Suppressants - None"
  label(data.tab1$sup.h2ra) <- "Acid Suppressants - H2RA"
  ...
  surv.unadjusted <- svydesign(ids = ~ ddiID, strata = ~ pain, nest = T, data = data.tab1)
  table1.unadjusted <- svyCreateTableOne(vars = covs, strata = "pain", test = F, data = surv.unadjusted)
  tab.1.unadjusted <- print(table1.unadjusted, nonnormal = c("age", "indexGFR", "bmi", "priorLos", "wbcBase", "hgbBase", "platBase",
                                                             "labclBase", "labkBase"), smd = TRUE, varLabels=T, test = F, showAllLevels = F, noSpaces =T,
                            contDigits = 1, formatOptions = list(drop0trailing = T))
  if (is.null(wts)) {
    tab.1 <- tab.1.unadjusted
  } else {
    ...
    surv.adjusted <- svydesign(ids = ~ ddiID, strata = ~ pain, weights = data[[wts]], nest = T, data = data.tab1)
    table1.adjusted <- svyCreateTableOne(vars = covs, strata = "pain", test = F, data = surv.adjusted)
    tab.1.adjusted <- print(table1.adjusted, nonnormal = c("age", "indexGFR", "bmi", "priorLos","wbcBase", "hgbBase", "platBase",
                                                           "labclBase", "labkBase"),
                            smd = TRUE, varLabels=T, test = F, showAllLevels = F, noSpaces =T,
                            contDigits = 1, formatOptions = list(drop0trailing = T))
...
table.1.doseCat <-function(data, wts = NULL, covs) {
  ## Make Table 1
  ...
  surv.unadjusted <- svydesign(ids = ~ ddiID, strata = ~ doseCat, nest = T, data = data.tab1)
  table1.unadjusted <- svyCreateTableOne(vars = covs, strata = "doseCat", test = F, data = surv.unadjusted)
  tab.1.unadjusted <- print(table1.unadjusted, smd = F, varLabels=T, test = F, showAllLevels = F, noSpaces =T,
                            contDigits = 1, formatOptions = list(drop0trailing = T))
  bal.formula <- reformulate(covs, response = "doseCat")
  bal.unadj <- bal.tab(bal.formula, data = data, binary = "std", s.d.denom = "pooled")
  max.smd.unadj <- bal.unadj$Balance.Across.Pairs$Max.Diff.Un
  max.smd.unadj <-c("-", round(max.smd.unadj,2))
  tab.1.unadjusted <- as.data.frame(tab.1.unadjusted)
  tab.1.unadjusted$`Max Abs SMD` <- max.smd.unadj
  tab.1.unadjusted <- tab.1.unadjusted %>% rownames_to_column(var = "row.names")
  
  if (is.null(wts)) {
    tab.1 <- tab.1.unadjusted
  } else {
    
    surv.adjusted <- svydesign(ids = ~ ddiID, strata = ~ doseCat, weights = data[[wts]], nest = T, data = data.tab1)
    table1.adjusted <- svyCreateTableOne(vars = covs, strata = "doseCat", test = F, data = surv.adjusted)
    tab.1.adjusted <- print(table1.adjusted, smd = F, varLabels=T, test = F, showAllLevels = F, noSpaces =T,
                            contDigits = 1, formatOptions = list(drop0trailing = T))
    bal.formula <- reformulate(covs, response = "doseCat")
    bal.adj <- bal.tab(bal.formula, data = data, binary = "std", s.d.denom = "pooled", weights = data[[wts]])
    max.smd.adj <- bal.adj$Balance.Across.Pairs$Max.Diff.Adj
    max.smd.adj <-c("-", round(max.smd.adj,2))
    tab.1.adjusted <- as.data.frame(tab.1.adjusted)
    tab.1.adjusted$`Max Abs SMD` <- max.smd.adj
    tab.1.adjusted <- tab.1.adjusted %>% rownames_to_column(var = "row.names")
    
    tab.1 <- tab.1.unadjusted %>%
      left_join(tab.1.adjusted, by = "row.names") 
    # %>%
    #   rename( " " = 1,
    #     "Low-Dose (Unweighted)" = 2,
    #          "Medium-Dose (Unweighted)" = 3,
    #          "High-Dose (Unweighted)" = 4,
    #          "Max Pairwise Absolute SMD (Unweighted)" = 5,
    #          "Low-Dose (Weighted)" = 6,
    #          "Medium-Dose (Weighted)" = 7,
    #          "High-Dose (Weighted)" = 8,
    #          "Max Pairwise Absolute SMD (Weighted)" = 9)
  }
  return(tab.1)
}

table.1.omeCat <-function(data, wts = NULL, covs) {
  ## Make Table 1
  ...
  data.tab1 <- data %>%
    mutate_if(is.integer, as.factor) %>% # need to label each level
    mutate(pain = factor(pain, labels = c("Oxycodone", "Ibuprofen"))) %>%
    mutate(omeCat.0 = factor(omeCat.0, labels = c("Received Opioids", "No Opioids Received")),
           omeCat.1 = factor(omeCat.1, labels = c("Not Q1", "Q1")),
           omeCat.2 = factor(omeCat.2, labels = c("Not Q2", "Q2")),
           omeCat.3 = factor(omeCat.3, labels = c("Not Q3", "Q3")),
           omeCat.4 = factor(omeCat.4, labels = c("Not Q4", "Q4")))
  
  surv.unadjusted <- svydesign(ids = ~ ddiID, strata = ~ pain, nest = T, data = data.tab1)
  table1.unadjusted <- svyCreateTableOne(vars = covs, strata = "pain", test = F, data = surv.unadjusted)
  tab.1.unadjusted <- print(table1.unadjusted, smd = F, varLabels=T, test = F, showAllLevels = F, noSpaces =T,
                            contDigits = 1, formatOptions = list(drop0trailing = T))
  bal.formula <- reformulate(covs, response = "pain")
  bal.unadj <- bal.tab(bal.formula, data = data, binary = "std", s.d.denom = "pooled")
  
  diff.smd.unadj <- bal.unadj$Balance$Diff.Un
  diff.smd.unadj <-c("-", round(diff.smd.unadj,2))
  tab.1.unadjusted <- as.data.frame(tab.1.unadjusted)
  tab.1.unadjusted$`SMD` <- diff.smd.unadj
  tab.1.unadjusted <- tab.1.unadjusted %>% rownames_to_column(var = "row.names")
  ...
  if (is.null(wts)) {
    ...
    bal.formula <- reformulate(covs, response = "pain")
    bal.adj <- bal.tab(bal.formula, data = data, binary = "std", s.d.denom = "pooled", weights = data[[wts]])
    diff.smd.adj <- bal.adj$Balance$Diff.Adj
    diff.smd.adj <-c("-", round(diff.smd.adj,2))
    tab.1.adjusted <- as.data.frame(tab.1.adjusted)
    tab.1.adjusted$`SMD` <- diff.smd.adj
    tab.1.adjusted <- tab.1.adjusted %>% rownames_to_column(var = "row.names")
...
table.1_low_risk <-function(data, wts, covs) {
  ## Make Table 1
