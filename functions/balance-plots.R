...
bal.plot.clean <- function(data, weights, strata, treatment, covs, subset = TRUE, main.title = NULL, long.names = NULL) {
  ...
  for (i in seq_along(data.list)) {
    ...
    ## Rename columns using covs
    # if long.names is null use this, otherwise use the vector of long.names provided by user
...
bal.plot.clean.bin <- function(data, weights, strata, treatment, covs, subset = TRUE, main.title = NULL, long.names = NULL) {
  ...
  if(is.null(long.names)){
    ...
    if (subset) {
      ...
      } else if (strata == "icuCurrent.cat") {
        var_name <- "ICU"
        if (i == 1) {
          plot_title <- main.title
        } else if (i == 2) {
          plot_title <- paste0(main.title, " (not in ", var_name, ")")
        } else if (i == 3) {
          plot_title <- paste0(main.title, " (in ", var_name, ")")
        }
      } else {
...
composite.bal.plot.bin <- function(plot.list, main.title, strata) {
  ...
  if (strata == "dm.bin") {
    ...
    subplot_titles <- c("Not in ICU", "In ICU")
  }
  else if (strata == "anyNtx.cat") {
    subplot_titles <- c("No Nephrotoxins", "Concomitant Nephrotoxins")
  }else {
    # Fallback for other strata
...
# For low risk subgorup (no ICU or periop vars)
bal.plot.clean.low.risk <- function(data, weights, strata, treatment, covs, subset = TRUE, main.title = NULL) {
  ...
  for (i in seq_along(data.list)) {
    ...
    long.names <- c("Age", "Sex", "Race - White", "Race -Black", "Race - Other", "Admisison Type", 
                    "Center - HUP", "Center - Presbyterian", "Center - Pennsylvania Hospital", "Presentation - ED", "Presentation ICU",
                    "Presentation - OR", "Presentation - Floor", "Presentation - Other", "Prior LOS", "Ventilator Status", 
                    "Prior Ventilator", "Heart Failure", "Myocardial Infarction", "Arrhythmia", "Atrial Fibrillation", "Valvular Diseas", 
                    "Stroke", "Peripheral Vascular Disease", "Pulmonary Circulation Disorder", "Chronic Pulmonary Disease", "Liver Disease", 
                    "Diabetes Mellitus - None", "Diabetes Mellitus - Non-complicated", "Diabetes Mellitus - Complicated", "Chronic Kidney Disease", 
                    "Weight Loss", "Fluid and Electrolyte Disorder", "Cancer - None", "Cancer - Non-complicated", "Cancer - Metastatic",
